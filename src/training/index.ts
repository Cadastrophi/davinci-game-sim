...
import type { ControlFrame, ExerciseMode, ExerciseSnapshot, Target, ToolPose, TrainingFacade, TrainingState, Vec3, Obstacle, IncisionSnapshot } from '../contracts';
import { clearance, createCollision, distance } from './collision';
import { createIncision, INCISION_PROTECTED } from './incision';
...
export function createTraining(initialPose: ToolPose = INITIAL_TOOL_POSE): TrainingFacade {
  ...
  const incision = createIncision();
  let completedIncision: IncisionSnapshot | null = null;
  ...
  const obstacles = (): readonly Obstacle[] => mode === 'obstacle' ? PROTECTED_BOXES : mode === 'incision' ? INCISION_PROTECTED : [];
  ...
  function reset(nowMs: number) {
    incision.reset(); completedIncision = null;
  ...
  return {
    start(nextMode, nowMs) {
      ...
      if (nextMode === 'incision' && INCISION_PROTECTED.some(box => clearance(applied.pose, box) <= 0.001)) {
        throw new Error('Move clear of protected volumes in Free practice before starting incision.');
      }
    ...
    step(frame, nowMs): TrainingState {
      ...
      const cutEligible = mode === 'incision' && phase === 'running' && fresh && frame.mode === 'tool';
      if (!cutEligible || !continuous || revisionChanged || interrupted || previous?.source !== frame.source) incision.interrupt();
      if (cutEligible && newSample) {
        incision.advance(applied.pose.positionMm);
        const cut = incision.snapshot(applied.pose.positionMm, true);
        if (cut.coverage01 === 1) { phase = 'completed'; completedIncision = { ...cut, contact: false }; }
      }
