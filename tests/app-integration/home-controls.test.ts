...
describe('home calibration and control feedback', () => {
  ...
  it('renders rapid pause/resume commands immediately and preserves pose', async () => {
    const h = setup();
    await h.controller.command({ type: 'start', mode: 'free' });
    h.move([9, 3, -4]);
    h.controller.tick();
    ...
    for (let i = 0; i < 20; i++) {
      commands.push(h.controller.command({ type: 'pause' }));
      commands.push(h.controller.command({ type: 'resume' }));
    }
    await Promise.all(commands);

    expect(h.uiRender).toHaveBeenCalledTimes(40);
    expect(h.uiRender.mock.lastCall?.[0].input.control.mode).toBe('tool');
    expect(h.state().exercise.phase).toBe('running');
    expect(h.state().applied.pose).toEqual(pose);
    await h.dispose();
  });
});
