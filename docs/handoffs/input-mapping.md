...
Native Node v24.19.0 smoke checks passed for calibration, collision-applied camera freeze, 100 duplicate camera snapshots, cumulative offsets, position/direction release rebasing, a missed stale gap, reconnect, and sparse-vector rejection. With the exact package-lock dependencies available, `tsc --noEmit` passed across this worktree and `vitest run src/input/mapping/index.test.ts` passed all 44 tests (Vitest 5.0.0, Node v24.19.0). The only typecheck corrections were deliberate snapshot-mutation tests, now expressed using `Reflect.set` rather than invalid readonly-to-mutable casts. Parent/E must still review and validate the combined serial/mapping/facade branches before integration approval. Behavioral coverage includes applied-vs-requested collision anchors, Space hold/release, duplicate render reads, strict stale boundary, missed stale gaps, stale release, blocked reset/resume, source/reconnect sequence restart, wrap/sign handling, antipodes, unavailable direction, invalid data/configuration, numeric overflow, and mutation isolation.
