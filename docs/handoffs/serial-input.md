...
- `git diff --check`: PASS.
- Full Vitest and TypeScript checks: PENDING. Dependency downloads were incomplete in the isolated worktree; no `node_modules/.bin` was available. An early fallback `pnpm test` began dependency resolution and was cancelled to avoid competing with the parent's installation. It did not execute tests.

This is reviewable draft work, not a fully validated READY handoff. Parent will run the committed Vitest suite and `tsc --noEmit` locally when dependencies finish, or ask E to run them on its dependency-ready machine before integration. No physical device or browser serial acceptance is claimed. Parent integrates the factory into `src/input/index.ts`, reviews these changes, publishes the PR and supplies it to E. Actual hardware still needs settings/packet/axis/rate verification.
